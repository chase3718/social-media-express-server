	...
	return {
		...
		plugins: [
			new WebpackShellPluginNext({
				...
				onBuildEnd: { scripts: ['npm run dev'], blocking: false, parallel: true },
			}),
		],
		module: { rules: [{ test: /\.(ts|js)$/, loader: 'ts-loader', options: {}, exclude: /node_modules/ }] },
		resolve: { extensions: ['.ts', '.js'], alias: { src: path.resolve(__dirname, 'src') } },
